...
**Security filters as intentional guardrails.** The three-layer secret filter (path filter, content scan, add filter) is a deliberate deviation from the "no heuristics" principle. Secrets are removed from the diff *before* the model sees them, and only files that pass both filters are ever staged. This prevents accidental leakage of credentials to the LLM and into git history. The model is never shown `.env`, `.pem`, `id_rsa`, or any file containing known token signatures (e.g. `sk-or-v1-`, `AKIA`, `ghp_`). Compiled binaries and build artifacts (`.exe`, `.dll`, `.so`, `.dylib`, `.o`, `.a`, `.lib`) are also excluded at the path filter layer.

**Commit strategy.** The `strategy` config field (`permissive` | `standard` | `strict`, default `standard`) adjusts the AI system prompt to control how selective the model is. `permissive` commits anything that passes the filters; `standard` skips obviously incomplete or debug-only diffs; `strict` demands clean, atomic, purposeful changes and rejects mixed-concern diffs. The path filter and `.quillignore` exclusions apply unconditionally before any strategy logic runs.
